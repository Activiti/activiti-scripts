...
DOCKERHUB_REPO=${4:-DOCKERHUB_REPO}
...
AUTH_SCOPE="repository:${DOCKERHUB_ORG}/${DOCKERHUB_REPO}:pull,push"
...
MANIFESTS_URL="https://${API_DOMAIN}/v2/${DOCKERHUB_ORG}/${DOCKERHUB_REPO}/manifests/${BASE_TAG}"
...
curl -X PUT -H "Content-Type: ${CONTENT_TYPE}" \
         ...
         -d "${MANIFEST}" \
         "https://${API_DOMAIN}/v2/${DOCKERHUB_ORG}/${DOCKERHUB_REPO}/manifests/${RELEASE_TAG}"

VERSIONS=$(curl -H "Authorization: Bearer ${TOKEN}" https://${API_DOMAIN}/v2/${DOCKERHUB_ORG}/${DOCKERHUB_REPO}/tags/list)
