    ...
    while read REPO_LINE_INNER;
     do REPO_ARRAY_INNER=($REPO_LINE_INNER)
       ...
       if [ "${COUNTER}" -eq "${INNER_COUNTER}" ];
         then
           echo "CHECKING THAT ${REPO} VERSION IS ${REPO_ARRAY_INNER[1]}"
       ...
       if [ "${COUNTER}" -gt "${INNER_COUNTER}" ];
         then
           echo "CHECKING THAT ${REPO} USES ${PROP_INNER}.version ${REPO_ARRAY_INNER[1]}"
           ...
           PROPERTY_VERSION=$(xmllint --xpath "//*[local-name()='properties']/*[local-name()='${PROP_INNER}.version']/text()" "${SRC_DIR:-$HOME/src}/$REPO/pom.xml" 2>/dev/null) || true
           if [ -z "${PROPERTY_VERSION}" ];
             then
               ...
               if [ "${PROPERTY_VERSION}" = "${REPO_ARRAY_INNER[1]}" ]
                 then
                   echo "${REPO_INNER} ${VERSION_INNER} IS USED IN ${PROP_INNER}.version PROPERTY OF ${REPO}"
                   VERSION_USED=true
               else
                 echo "${REPO} USES ${PROPERTY_VERSION} FOR ${PROP_INNER}.version WHEN EXPECTED VERSION IS ${VERSION_INNER}"
                 exit 1
               fi
           fi
